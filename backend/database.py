...
class Database:
    def __init__(self, nome_banco):
        ...
        self.conn = None
        self.cursor = None
        self.connect()  # Conecta e inicializa o cursor imediatamente
    ...
    def atualizar(self, nome_tabela: str, updates: dict[str, any], condicao: str, params: tuple = ()):
        updates_str = ', '.join([f"{col} = ?" for col in updates.keys()])
        query = f"UPDATE {nome_tabela} SET {updates_str} WHERE {condicao}"
        self.cursor.execute(query, list(updates.values()) + list(params))
        self.conn.commit()
